...
this.EXPORTED_SYMBOLS = ["wallpaperTourType"];
...
var wallpaperTourType = {
   ...
   */
  check() {
    const PREF_TOUR_TYPE = "browser.wallpaper.tour-type";
    const PREF_SEEN_TOURSET_VERSION = "browser.wallpaper.seen-tourset-version";
    const TOURSET_VERSION = Services.prefs.getIntPref("browser.wallpaper.tourset-version");

    if (!Services.prefs.prefHasUserValue(PREF_SEEN_TOURSET_VERSION)) {
      // User has never seen an wallpaper tour, present the user with the new user tour.
      ...
      Services.prefs.setBoolPref("browser.wallpaper.hidden", false);
    }
